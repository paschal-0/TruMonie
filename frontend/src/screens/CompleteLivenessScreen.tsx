...
export const CompleteLivenessScreen: React.FC = () => {
  ...
  const captureStep = async () => {
    if (!cameraRef.current || capturedAll) return;
    try {
      setCapturing(true);
      setError(null);
  ...
  const submit = async () => {
    if (!sessionId || !token || frames.length === 0) return;
    try {
      setSubmitting(true);
      setError(null);
  ...
  return (
    <View style={styles.container}>
      <ThemedText style={styles.title}>Complete Liveness</ThemedText>
      <ThemedText style={styles.subtitle}>
      ...
      {capturing || submitting ? (
        ...
        <GradientButton title="Done" onPress={() => navigation.navigate('Main')} style={styles.cta} />
      ) : (
        <>
          <Pressable
